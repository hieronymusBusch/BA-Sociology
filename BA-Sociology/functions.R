...
# OLS regress a var against a previously defined list of variables, 
   ...
   # for INTERACTIONS: var1 = var2*var3
OLSvarlist3 <- function(var1, var2, var3, varList, controlList){
  dfoutput <- data.frame()
  dfoutput1 <- data.frame()
  dfoutput2 <- data.frame()
  dfoutput3 <- data.frame()
  input2 <- c(var1,var2,var3,controlList)
  for(a in varList) {
    inputvar1 <- a
    f <- as.formula(paste(inputvar1,paste(input2, collapse = " + "), sep = "~"))
    f <- as.formula(paste(inputvar1,paste(input2, collapse = " + "), sep = "~"))
    ...
    e <- data.frame(coefficientReg3 = b[4, 1])
    dfoutput1 <- rbind(dfoutput1,c)
    dfoutput2 <- rbind(dfoutput2,d)
    dfoutput3 <- rbind(dfoutput3,e)
  }
  ...
  dfoutput$coefficientReg3 <- round(dfoutput3$coefficientReg3, digits = 5)
  dfoutput
}
...
# SAR regress a var against a previously defined list of variables, 
 # output as df of avg total effects & rho, including controls
 # for INTERACTIONS: var1 = var2*var3
SARvarlist3 <- function(var1, var2, var3, varList, controlList){
  dfoutput <- data.frame()
  dfoutput1 <- data.frame()
  dfoutput2 <- data.frame()
  dfoutput3 <- data.frame()
  dfoutput4 <- data.frame()
  input2 <- c(var1,var2,var3,controlList)
  for(a in varList) {
    inputvar1 <- a
    f <- as.formula(paste(inputvar1,paste(input2, collapse = " + "), sep = "~"))
    ...
    e <- data.frame(coefficientReg3 = as.numeric(b["total3"]))
    dfoutput1 <- rbind(dfoutput1,c)
    dfoutput3 <- rbind(dfoutput3,d)
    dfoutput4 <- rbind(dfoutput4,e)
  ...
  dfoutput$coefficientReg3 <- round(dfoutput4$coefficientReg3, digits = 5)
  dfoutput$rho <- dfoutput2$rho
  dfoutput
}
